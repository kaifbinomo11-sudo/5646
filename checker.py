...
def check_cookie(
    ...
    bulk_mode=True    → tighter timeout, no NFToken (faster for bulk).
    generate_token    → set False in bulk mode; token generated on demand.
    force_direct      → skip all proxies, go direct (used for admin direct mode).
    user_proxies      → list of proxy URL strings; use these instead of admin pool.
                        If None and not force_direct → use global proxy_manager pool.
    """
    import concurrent.futures as _cf
    ...
    def _fetch_account():
        ...
        Proxy selection priority (per-check):
          force_direct=True  → always direct, regardless of global proxy toggle.
          user_proxies list  → use caller-supplied proxies (user's own pool).
          otherwise          → use admin's global proxy_manager pool.
        """
        # ── Determine proxy mode for this check ──────────────────────────────
        _user_proxy_dicts: list | None = None   # None = use admin pool
        if force_direct:
            _proxy_active = False
        elif user_proxies is not None:
            _user_proxy_dicts = [{"http": u, "https": u} for u in user_proxies if u]
            _proxy_active = bool(_user_proxy_dicts)
        else:
        ...
        def _one_attempt(sess, proxy_dict, url, *, _is_user_proxy: bool = False):
            """
            ...
            _is_user_proxy=True skips proxy_manager score callbacks (user-owned proxies).
            """
            ...
            try:
                ...
                if _is_rate_limited(r.status_code, r.text):
                    if proxy_dict and _proxy_manager and not _is_user_proxy:
    ...
    try:
        ...
        if acct_status == "ERROR" or not acct_text:
            if user_proxies is not None and not force_direct:
                return {
                    "status": "error",
                    "message": (
                        "Your proxy failed to reach Netflix. "
                        "Please check that your proxy is working and try again, "
                        "or remove it via /myproxy and use tokens instead."
                    ),
                }
