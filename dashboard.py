...
function statusPill(s) {
  ...
  return `<span class="pill ${esc(cls)}">${labels[s] || esc(s)}</span>`;
}
...
async function refresh() {
  try {
    ...
    if (!r.ok) { console.warn('Dashboard: stats fetch failed', r.status); return; }
    ...
    if (d.activity && d.activity.length > 0) {
      tbody.innerHTML = d.activity.map(a =>
        `<tr><td style="color:#555">${esc(a.time)}</td><td>${statusPill(a.status)}</td><td style="color:#888;font-size:0.8rem">${esc(a.source)}</td></tr>`
      ).join('');
    }
  } catch(e) {}
}
...
def _check_token() -> None:
    """Abort with 401 if DASHBOARD_TOKEN is set and the request doesn't supply it."""
